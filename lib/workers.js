...
workers.validateCheckData = function(originalCheckData) {
	...
	originalCheckData.timeoutSeconds % 1 === 0 && originalCheckData.timeoutSeconds >= 1 && originalCheckData.timeoutSeconds <= 5 ? originalCheckData.timeoutSeconds : false;

	// set the keys which may not be set yet (unseen by workers)
	...
	if (originalCheckData.id &&
		...
		console.error("Error : A Check is not properly formatted, skipping it.");
	}
};
...
// special logic for accomodating a check that has never been tested before (don't send a message)
...
workers.alertUserToStatusChange = function(newCheckData) {
	...
	helpers.sendTwilioSms(newCheckData.userPhone,msg,function(err){
		if(!err) {
			console.error("Error : User was alerted to a status change in their check, via sms");
		} else {
			console.error("Error : Error sending SMS alert to user who had a state change in one of their checks.")
		}
	});
};
...
workers.log = function(originalCheckData,checkOutcome,state,alertWarranted,timeOfCheck) {
	...
	_logs.append(logFileName,logString,function(err){
		if(!err) {
			console.error("Logging to file succeeded.");
		} else {
				...
				_logs.compress(logId,newFileId,function(err){
					if(!err) {
						...
						_logs.truncate(logId,function(err){
							if(!err){
								console.error("Success truncating logFile");
							} else {
...
workers.init = function() {
	...
	// call the checkLoop so checks execute later
	workers.checkLoop();

	// compress all pre-existing logs immediately
	workers.rotateLogs();
